...
export default function useLocalStorage(keyFromLocalStorage) {
  ...
  useEffect(() => {
    ...
    if (res) {
      ...
      localStorage.setItem(keyFromLocalStorage, JSON.stringify([]));
    }
    // эти начальные данные для локал сторадж, чтобы сразу создавался пустой массив. Их нельзя в таком виде положить в отрисовку
  }, []);
